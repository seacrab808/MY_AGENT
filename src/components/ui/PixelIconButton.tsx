...
export function PixelIconButton({
  ...
  return (
    <button
      className={`font-cute font-bold text-base leading-none min-w-[44px] min-h-[44px] flex items-center justify-center px-2 py-2 border-2 border-pixel-border rounded-[12px] shadow-[var(--pixel-bevel)] active:shadow-[var(--pixel-bevel-active)] transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed ${toneClasses[tone]} ${className}`}
      {...rest}
    >
      {children}
    </button>
